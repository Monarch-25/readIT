/**
 * Live integration smoke test against the REAL MLX Qwen3-TTS server.
 *
 * Skips unless a server is running and healthy (default 127.0.0.1:8901;
 * override with MLX_SERVER). Exercises the full new flow: popup connect ->
 * open player window -> paste text -> real synthesis via
 * /v1/audio/speech/batch -> real playback in the player window.
 *
 * Run: MLX_SERVER=http://127.0.0.1:8901 node --test e2e/real_mlx.test.mjs
 */
...
const SERVER = process.env.MLX_SERVER || 'http://127.0.0.1:8901';
...
const SAMPLE_TEXT = [
  ...
  'He was expected at the Merchant\u2019s Oath by nine, and the summons had arrived in the same hand that had signed his late father\u2019s contracts.',
  ...
  'A man with hands like dockworkers\u2019 tools was waiting in the booth, and a bottle of brandy stood between two glasses.',
  '\u201cYou\u2019re late,\u201d said the man.',
  '\u201cYou\u2019re not the one who sent for me,\u201d Roland said, sitting down without being asked.',
  'Outside, the rain found its rhythm against the window, and the room went quiet around them.',
].join('\n\n');
...
async function probeHealth() {
  ...
  try {
    ...
    return { ok: j.ok === true, voices: j.voices };
  } catch (e) {
...
if (liveHealth.ok) console.log(`# live MLX server healthy (${liveHealth.voices} voices) at ${SERVER}`);
...
test('player reads pasted text with REAL Qwen3-TTS audio', { skip: !liveHealth.ok }, async () => {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'vr-real-'));
  ...
  assert.equal(voiceCount, liveHealth.voices, 'voice list should come from the real model');
  ...
  await sleep(1500);
  assert.equal(await progress(), frozen, 'progress must not advance while paused');
  await playerPage.locator('#pause').click();
  await waitFor(playerPage, '#state', (t) => (t || '').startsWith('Reading'), 15000);

  // speed applies to playback
